...
import { ISaveTaskBody, ISubtask, ITask, ITaskUpdateReq } from "@kanban/sdk/bin/link";
...
class TaskController {
  static saveTask: RequestHandler<{ boardId: string }, {}, ISaveTaskBody> = asyncErrorHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      res.status(CODE_200).json(new ApiResponse(CODE_200, { task }, TASK_CREATE_SUCCESS));
    }
  );

  static updateTask: RequestHandler<{ taskId: string }, {}, ITaskUpdateReq> = asyncErrorHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      res.status(CODE_200).json(new ApiResponse(CODE_200, null, TASK_UPDATE_SUCCESS));
    }
  );

  static deleteTask: RequestHandler<{ taskId: string }, {}, {}> = asyncErrorHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      const { taskId } = req.params;
      const task = await Task.findById(taskId) as ITask;
      ...
      res.status(CODE_200).json(new ApiResponse(CODE_200, {}, TASK_DELETE_SUCCESS));
    }
  );
}
