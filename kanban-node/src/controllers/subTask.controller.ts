...
import { CODE_200, CODE_404, SUB_TASK_CREATE_SUCCESS, SUBTASK_DELETE_SUCCESS, SUBTASK_NOT_FOUND, SUBTASK_UPDATE_SUCCESS, Task_NOT_FOUND } from "../utils/translations";
...
class SubTaskController {
  static saveSubTask: RequestHandler<{ taskId: string }, {}, ISubTaskRequest> = asyncErrorHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      res.status(CODE_200).json(new ApiResponse(CODE_200, { subTask }, SUB_TASK_CREATE_SUCCESS));
    }
  );

  static updateSubTask: RequestHandler<{ subtaskId: string }, {}, ISubTaskUpdateReq> = asyncErrorHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      res.status(CODE_200).json(new ApiResponse(CODE_200, {}, SUBTASK_UPDATE_SUCCESS));
    }
  );
  ...
  static deleteSubTask: RequestHandler<{ subtaskId: string }, {}, {}> = asyncErrorHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      res.status(CODE_200).json(new ApiResponse(CODE_200, {}, SUBTASK_DELETE_SUCCESS));
    }
  );
}
